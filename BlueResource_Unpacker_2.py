import os
makesure = 203000 # a size to use to make sure a BlueResource file being read is valid
ddsh = b'\x44\x44\x53\x20\x7C\x00\x00\x00' # dds checker
fsb = b'\x46\x53\x42\x34\x01\x00\x00\x00' # fsb checker
sigcheck = b'\x78\x56\x34\x12' # BlueResource checker
ext1 = ".dds" # for dds extension
ext2 = ".bin" # for bin extension
ext3 = ".fsb" # for fsb extension
extension = ".BlueResourcePC_Windows" # BlueResource container file extension
...
def arga(): # Unpacking
    ...
    for file in files_with:
        try:
            sizecheck = os.path.getsize(file) # get the size of the current file
            if sizecheck < makesure: # if the current file being opened is smaller than the valid size check I implemented
                helpo(skippedfile, ERROR_WRONG, listed_file=file)
                continue
            with open(file, "r+b") as f1: # The file
                ...
                entrysecs = int.from_bytes(f1.read(4), "little") # amount of file entry sections across the entire container file, specified at offset 12 of each container file
                for j in range(0, entrysecs): # main loop to be used for file entry sections
                    ...
                    entryamount = int.from_bytes(f1.read(4), "little") # total file entries within the current file entry section.
                    ...
                    for i in range(0, entryamount): # for metadata within file entry section 1 onwards
                        unk3 = f1.read(8) # unknown
                        size = int.from_bytes(f1.read(4), "little") # file size for current file entry within the current file entry section
                        unk4 = f1.read(4) # unknown
